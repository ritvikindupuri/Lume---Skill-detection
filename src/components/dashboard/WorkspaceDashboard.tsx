  ...
  if (recent.length < 2)
    return (
      <div className="flex h-44 items-center justify-center text-sm text-muted-foreground">
        Scan two or more skills to see risk over time.
      </div>
    );
  const coords = recent.map((scan, index) => ({
        ...
        <div className="absolute inset-y-0 left-12 right-0">
          <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="h-full w-full">
            {[0, 25, 50, 75, 100].map((n) => (
              <line
                ...
                strokeDasharray={n === 0 || n === 100 ? "0" : "1.5 1.5"}
              />
            ))}
            <polyline
              points={points}
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{
                    backgroundColor: VERDICT_COLORS[scan.verdict] ?? "#6366f1",
                    transform: `translate(-50%,-50%) scale(${hovered === index ? 1.6 : 1})`,
                  }}
                />
              </button>
            );
          ...
          {active && activeCoords && (
            <div
              ...
              style={{
                left: `${Math.min(Math.max(activeCoords.x, 12), 88)}%`,
                top: `${activeCoords.y}%`,
                transform: `translate(-50%, ${activeCoords.y < 30 ? "12px" : "calc(-100% - 12px)"})`,
              }}
            >
              <p className="truncate font-medium">{active.artifact_name}</p>
              <p className="mt-1 text-muted-foreground">
                {new Date(active.scanned_at).toLocaleDateString()}
              </p>
              <div className="mt-2 flex items-center justify-between">
                ...
                <span
                  className="rounded-full px-2 py-0.5 capitalize text-[10px] font-medium"
                  style={{
                    backgroundColor: `${VERDICT_COLORS[active.verdict] ?? "#6366f1"}20`,
                    color: VERDICT_COLORS[active.verdict] ?? "#6366f1",
                  }}
                >
                  {active.verdict}
                </span>
      ...
      <div className="mt-2 flex items-center justify-between pl-12 text-[10px] text-muted-foreground">
        ...
        <div className="flex items-center gap-3">
          {(["clean", "suspicious", "malicious"] as const).map((v) => (
            <span key={v} className="flex items-center gap-1 capitalize">
              <span
...
export function WorkspaceDashboard() {
  ...
  const [tab, setTab] = useState<"overview" | "approvals" | "policy" | "history" | "checks">(
    "overview",
  );
  ...
  const refresh = async () => {
    ...
    if (data.settings)
      setPolicy({
        acceptableScore: data.settings.acceptable_score,
        maliciousScore: data.settings.malicious_score,
        blockOnCritical: data.settings.block_on_critical,
      });
    if (data.organization)
      setChecks(await listChecksFn({ data: { organizationId: data.organization.id } }));
  };
  ...
  const establishWorkspace = async () => {
    ...
    setMessage(null);
    try {
      await createWorkspaceFn({ data: { name: workspaceName } });
      await refresh();
    } catch (error) {
  ...
  const savePolicy = async () => {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Could not save the policy.");
    } finally {
    ...
    try {
      for (const file of Array.from(files)) {
        setThinking({
          artifact: file.name,
          reasoning: "",
          steps: [{ label: "Reading the skill files", done: false }],
        });
        ...
        setThinking(
          (current) =>
            current && {
              ...
              steps: [
                {
                  label: `Read ${artifact.files.length} file${artifact.files.length === 1 ? "" : "s"}: ${fileList}`,
                  done: true,
                },
                {
                  label: `Running ${RULES.length + compiledChecks.length} deterministic checks`,
                  done: false,
                },
              ],
            },
        );
        const deterministic = await scanArtifact(
          artifact.name,
          artifact.files,
          policy,
          compiledChecks,
          ...
          (current) =>
            current && {
              ...
              steps: [
                ...
                {
                  label: `Deterministic checks complete · ${deterministic.findings.length} finding${deterministic.findings.length === 1 ? "" : "s"}`,
                  done: true,
                },
        ...
        const ai = await streamAiScan(
          {
            ...
            deterministicFindings: deterministic.findings.map(
              ({ ruleId, title, severity, file: findingFile, line, evidence }) => ({
                ruleId,
                title,
                severity,
                file: findingFile,
          ...
          (text) =>
            setThinking(
              (current) => current && { ...current, reasoning: current.reasoning + text },
            ),
        );
        setThinking(
          (current) =>
            current && {
              ...
              steps: current.steps.map((step, index) =>
                index === current.steps.length - 1
                  ? {
                      label: `GPT review complete · ${ai.findings.length} additional finding${ai.findings.length === 1 ? "" : "s"}`,
                      done: true,
                    }
                  : step,
              ),
        ...
        await saveScanFn({
          data: {
            ...
            findings: result.findings.map((finding) => ({
              ...finding,
              category: LAYER_LABEL[finding.layer],
            })),
          },
        });
      }
      ...
      await refresh();
      setMessage(
        `${completed.length} skill${completed.length === 1 ? "" : "s"} analyzed and saved.`,
      );
    } catch (error) {
      setMessage(
        error instanceof ArtifactError || error instanceof Error
          ? error.message
          : "Could not analyze this batch.",
      );
    } finally {
      setScanning(false);
  ...
  if (!workspace?.organization)
    return (
      <main className="min-h-screen bg-background px-6">
        <header className="mx-auto flex h-16 max-w-6xl items-center justify-between">
          <Logo />
        ...
        <section className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-md flex-col justify-center pb-24">
          ...
          <p className="mt-3 text-muted-foreground">
            This keeps your policy, checks, and scan history together.
          </p>
          <Input
            className="mt-8 h-11 bg-card"
            placeholder="Workspace name"
            value={workspaceName}
            onChange={(event) => setWorkspaceName(event.target.value)}
            onKeyDown={(event) => event.key === "Enter" && void establishWorkspace()}
          />
          {message && <p className="mt-3 text-sm text-critical">{message}</p>}
          <Button className="mt-4 h-11 rounded-full" onClick={() => void establishWorkspace()}>
        ...
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          <div>
            ...
            <h1 className="mt-2 font-display text-4xl font-semibold sm:text-5xl">
              Risk, without the noise.
            </h1>
            <p className="mt-3 text-muted-foreground">
              {RULES.length} built-in checks, {checks.filter((check) => check.enabled).length} of
              your own, plus GPT intent review.
            </p>
          </div>
          <input
            ref={inputRef}
          ...
          >
            {scanning ? <LoaderCircle className="animate-spin" /> : <Upload />}{" "}
            {scanning ? "Analyzing…" : "Scan multiple skills"}
          </Button>
        </div>
        {message && (
        ...
        <div className="mt-8 inline-flex rounded-full border border-border bg-card p-1 text-sm">
          {(
            ...
            <button
              ...
              className={`rounded-full px-4 py-1.5 transition-colors ${tab === value ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
            >
              {label}
            </button>
          ))}
        ...
        {tab === "approvals" ? (
          ...
          <div className="mx-auto mt-8 max-w-3xl space-y-8">
            <section className="grid gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-3">
              {[
                { label: "Scans", value: summary.total },
                { label: "Average risk", value: `${summary.average}/100` },
                { label: "Blocked", value: summary.blocked },
              ].map((metric) => (
                <div key={metric.label} className="bg-card p-6">
                  <p className="label-mono">{metric.label}</p>
                  <p className="mt-3 font-display text-4xl font-medium">{metric.value}</p>
                </div>
              ))}
            </section>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate font-medium">
                            {scan.metadata.name ?? scan.artifactName}
                          </p>
                          <p className="mt-1 text-xs text-muted-foreground">
                            {scan.findings.length} findings · {scan.durationMs} ms
                            {scan.ai ? ` · ${scan.ai.findings} from AI` : ""}
                          </p>
                        </div>
                        <span className={`font-mono text-lg ${verdictClass[scan.verdict]}`}>
                          {scan.score}
                        </span>
                      </div>
                    </div>
            ...
            <section className="rounded-xl border border-border bg-card p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="label-mono">Risk trend</p>
                  <h2 className="mt-2 font-display text-xl font-medium">Scores over time</h2>
                </div>
                <FileScan className="text-muted-foreground" />
              </div>
              <div className="mt-8">
                <Trend scans={workspace.scans} />
              </div>
            </section>
            ...
            <section className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="flex items-center justify-between border-b border-border px-6 py-5">
                <div>
                  ...
                  <h2 className="mt-2 font-display text-xl font-medium">Latest analyzed skills</h2>
                </div>
              ...
              {workspace.scans.length === 0 ? (
                <div className="flex min-h-52 flex-col items-center justify-center px-6 text-center">
                  ...
                  <p className="mt-3 text-sm text-muted-foreground">
                    No scans yet. Upload skills with the button above to begin.
                  </p>
                </div>
              ) : (
                    ...
                    >
                      <span className="min-w-0">
                        ...
                        <span className="mt-1 block text-xs text-muted-foreground">
                          {new Date(scan.scanned_at).toLocaleString()} · {scan.findings_count}{" "}
                          findings
                          {policy.blockOnCritical &&
                          ((scan.severity_counts as Record<string, number> | null)?.["critical"] ??
                            0) > 0 ? (
                            <span className="ml-2 rounded-full bg-critical/10 px-2 py-0.5 font-medium text-critical">
                              Auto-blocked · critical
                            </span>
                          ...
                          {scan.containment === "cleared" && (
                            <span className="ml-2 rounded-full bg-safe/10 px-2 py-0.5 font-medium text-safe">
                              Cleared
                            </span>
                          )}
                        </span>
                      ...
                      <span
                        className={`self-center text-right text-sm font-medium capitalize ${verdictClass[scan.verdict as keyof typeof verdictClass] ?? "text-foreground"}`}
                      >
                        {scan.verdict}
                      </span>
