...
function toCounts(value: unknown): Record<Severity, number> {
  ...
  return { critical: Number(raw.critical ?? 0), high: Number(raw.high ?? 0), medium: Number(raw.medium ?? 0), low: Number(raw.low ?? 0) };
}
  ...
  return (
    <>
      <div>
        ...
        <Slider className="mt-4" min={0} max={98} step={1} value={[policy.acceptableScore]} onValueChange={([value]) => onChange({ ...policy, acceptableScore: Math.min(value ?? policy.acceptableScore, policy.maliciousScore - 1) })} />
      </div>
      <div className="mt-8">
        <div className="flex justify-between text-sm">
        ...
        <Slider className="mt-4" min={1} max={100} step={1} value={[policy.maliciousScore]} onValueChange={([value]) => onChange({ ...policy, maliciousScore: Math.max(value ?? policy.maliciousScore, policy.acceptableScore + 1) })} />
      </div>
      <div className="mt-8 flex items-center justify-between gap-4">
        ...
        <Switch checked={policy.blockOnCritical} onCheckedChange={(checked) => onChange({ ...policy, blockOnCritical: checked })} />
      </div>
    </>
  );
}
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[380px_1fr]">
      <aside className="h-fit rounded-xl border border-border bg-card p-6">
        <div className="flex items-center gap-3"><Settings2 className="text-primary" /><div><p className="font-medium">Risk policy</p><p className="text-xs text-muted-foreground">Move a slider to see the effect instantly.</p></div></div>
        <div className="mt-8"><PolicyControls policy={policy} onChange={onChange} /></div>
        <Button className="mt-8 w-full rounded-full" disabled={saving || !canSave} onClick={onSave}>{saving && <LoaderCircle className="animate-spin" />}Save policy</Button>
        {!canSave && <p className="mt-3 text-center text-xs text-muted-foreground">Only workspace administrators can change policy.</p>}
      </aside>
      ...
      <div className="space-y-8">
        <section className="grid gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-3">
          {[
            ...
            { label: "Review band", value: `${Math.max(policy.maliciousScore - policy.acceptableScore, 0)} pts`, tone: "text-foreground" },
          ].map((metric) => (
            <div key={metric.label} className="bg-card p-6">
              <p className="label-mono">{metric.label}</p>
              <p className={`mt-3 font-display text-4xl font-medium ${metric.tone}`}>{metric.value}</p>
            </div>
          ))}
        </section>
          ...
          {preview.length > 0 && (
            <div className="divide-y divide-border">
              {preview.map(({ scan, next }) => (
                <div key={scan.id} className="grid grid-cols-[1fr_auto] items-center gap-4 px-6 py-4 sm:grid-cols-[1fr_130px_90px]">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      ...
                      {next.verdict !== scan.verdict && <span className="ml-2 rounded-full bg-secondary px-2 py-0.5 font-medium text-foreground">would change</span>}
                    </p>
                  </div>
                  <span className={`self-center text-right text-sm font-medium capitalize ${verdictClass[next.verdict] ?? ""}`}>{next.verdict}</span>
                  <span className="hidden self-center text-right font-mono text-lg sm:block">{next.score}</span>
                </div>
              ))}
            </div>
          )}
