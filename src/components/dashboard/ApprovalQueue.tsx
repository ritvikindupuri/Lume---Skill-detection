...
export function ApprovalQueue({ organizationId, canReview, onChanged }: Props) {
  ...
  const refresh = async () => {
    try {
      setQueue(await loadQueue({ data: { organizationId } }));
    } catch {
      setQueue({ findings: [], containment: [] });
    }
  };
  ...
  const resolveFinding = async (
    ...
    const toastId = toast.loading(
      decision === "approve" ? "Approving this finding…" : "Reverting to unreviewed…",
    );
    try {
      const result = await review({
        data: { findingId, scanId, status: decision === "approve" ? "confirmed" : "open" },
      });
      await refresh();
      await onChanged?.();
      if (decision === "approve") {
        toast.error(`Finding approved · score ${result.score}, verdict ${result.verdict}`, {
          ...
          description:
            result.containment === "quarantined"
              ? "The skill is now quarantined."
              : "The scan was re-scored with this risk counted.",
        });
      } else {
        toast.success("Sent back for review", {
          id: toastId,
          description: "The finding is unreviewed again.",
        });
      }
    } catch (cause) {
      toast.error(cause instanceof Error ? cause.message : "Could not record this decision.", {
        id: toastId,
      });
    } finally {
      setBusy(null);
  ...
  const resolveContainment = async (scanId: string, decision: "approve" | "reject") => {
    setBusy(scanId);
    const toastId = toast.loading(
      decision === "approve" ? "Quarantining this skill…" : "Rejecting the recommendation…",
    );
    try {
      ...
      await onChanged?.();
      if (decision === "approve")
        toast.error("Skill quarantined", {
          id: toastId,
          description: "You approved the AI containment call.",
        });
      else
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 font-display text-xl font-medium">
              Nothing changes until a human signs off
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Findings marked as a real risk and AI containment calls wait here. Approving re-scores
              the scan; reverting puts it back to unreviewed.
            </p>
          </div>
          <Button
            variant="outline"
          ...
          {queue.containment.map((scan) => (
            <div key={scan.id} className="rounded-xl border border-border bg-card p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className="flex items-center gap-1.5 rounded-full bg-medium/10 px-3 py-1 text-xs font-medium text-medium">
              ...
              <div className="mt-4 rounded-lg border border-border bg-secondary/60 p-4">
                <p className="flex items-center gap-2 text-xs font-medium">
                  <Bot className="size-3.5 text-primary" /> Analysis log ·{" "}
                  {scan.ai_recommendation_confidence}% confidence
                </p>
                <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-muted-foreground">
                  {scan.ai_recommendation_reason ||
                    "The reviewer recommended containment without a written rationale."}
                </p>
              </div>
              {canReview && (
                <div className="mt-4 flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() => void resolveContainment(scan.id, "approve")}
                  >
                    {busy === scan.id ? <LoaderCircle className="animate-spin" /> : <ShieldBan />}{" "}
                    Approve quarantine
                  </Button>
                  <Button
                    size="sm"
                    ...
                    className="rounded-full"
                    disabled={busy === scan.id}
                    onClick={() => void resolveContainment(scan.id, "reject")}
                  >
                    <ShieldCheck /> Reject
                  </Button>
                </div>
            ...
            <div key={finding.id} className="rounded-xl border border-border bg-card p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {finding.scanName} · {finding.file_path}:{finding.line_number}
                  </p>
                </div>
                <div className="text-right">
                  <p
                    className={`text-sm font-medium capitalize ${severityClass[finding.severity as Severity] ?? ""}`}
                  >
                    {finding.severity}
                  </p>
              ...
              <div className="mt-4 space-y-3 rounded-lg border border-border bg-secondary/60 p-4">
                ...
                {finding.aiReason && (
                  <div>
                    <p className="flex items-center gap-2 text-xs font-medium">
                      <Bot className="size-3.5 text-primary" /> AI reading of this skill
                    </p>
              ...
              {canReview && (
                <div className="mt-4 flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() => void resolveFinding(finding.id, finding.scan_id, "approve")}
                  >
                    {busy === finding.id ? (
                  ...
                  <Button
                    ...
                    className="rounded-full"
                    disabled={busy === finding.id}
                    onClick={() => void resolveFinding(finding.id, finding.scan_id, "revert")}
                  >
                    <Undo2 /> Revert
                  </Button>
                </div>
      ...
      {!canReview && !empty && (
        <p className="text-xs text-muted-foreground">
          Viewers can see the queue but cannot approve or revert.
        </p>
      )}
    </div>
  );
