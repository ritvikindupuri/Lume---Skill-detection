...
export function ScanHistory({ scans, canEdit, selectedId, onOpen, onChanged }: Props) {
  ...
  const toggleCompare = (id: string) =>
    setCompare((current) => (current.includes(id) ? current.filter((item) => item !== id) : [...current, id].slice(-2)));

  const pair = compare.map((id) => scans.find((scan) => scan.id === id)).filter((scan): scan is HistoryRow => Boolean(scan));
  ...
  const confirmDelete = async (scan: HistoryRow) => {
    ...
    try {
      ...
      setCompare((current) => current.filter((id) => id !== scan.id));
      await onChanged();
      ...
      {pair.length === 2 && (
        <section className="overflow-hidden rounded-xl border border-border bg-card">
          ...
          <div className="border-t border-border px-6 py-4">
            <Button variant="outline" size="sm" className="rounded-full" onClick={() => setCompare([])}>Clear comparison</Button>
          </div>
        </section>
      )}
      ...
      <section className="overflow-hidden rounded-xl border border-border bg-card">
        <div className="border-b border-border px-6 py-5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Tick two scans to compare them, open one to review its findings, or delete a record you no longer need.
            {compare.length === 1 && " Pick one more to compare."}
          </p>
        </div>
        {scans.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {scans.map((scan) => (
              <div key={scan.id} className={`flex items-center gap-4 px-6 py-4 ${selectedId === scan.id ? "bg-secondary" : ""}`}>
                <input
                  type="checkbox"
                  aria-label={`Compare ${label(scan)}`}
                  checked={compare.includes(scan.id)}
                ...
                {canEdit && (
                  <Button variant="ghost" size="icon" title="Delete scan" disabled={deleting === scan.id} onClick={() => void confirmDelete(scan)}>
