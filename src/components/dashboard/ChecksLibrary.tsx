...
export function ChecksLibrary({ organizationId, canEdit, checks, onChanged }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      new RegExp(draft.pattern, "i");
      setPatternError(null);
    } catch {
      setPatternError("This pattern is not valid.");
    }
  }, [draft.pattern]);
  ...
  const filtered = useMemo(() => {
    ...
    return RULES.filter((rule) =>
      [rule.id, rule.title, rule.rationale, LAYER_LABEL[rule.layer]]
        .join(" ")
        .toLowerCase()
        .includes(needle),
    );
        ...
        <section className="rounded-xl border border-border bg-card">
          <div className="flex flex-col gap-4 border-b border-border px-6 py-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-2 flex items-center gap-2 font-display text-xl font-medium">
                <Sparkles className="size-5 text-primary" /> Write checks from your own skills
              </h2>
              <p className="mt-1 text-sm text-muted-foreground">
                Upload one or more skills. A separate AI pass reads them, compares against every
                existing check, and proposes new patterns grounded in what it actually found.
              </p>
            </div>
            <input
              ref={suggestInputRef}
              ...
              className="hidden"
              onChange={(event) => void suggestFromFiles(event.target.files)}
            />
            <Button
              variant="outline"
              className="rounded-full"
              disabled={suggesting}
              onClick={() => suggestInputRef.current?.click()}
            >
              {suggesting ? <LoaderCircle className="animate-spin" /> : <Upload />}{" "}
              {suggesting ? "Reading skills…" : "Upload skills"}
            </Button>
          </div>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        ...
                        · {LAYER_LABEL[suggestion.layer]} · confidence {suggestion.confidence}%
                      </p>
                    </div>
                    <div className="flex shrink-0 items-center gap-2">
                      <Button
                        size="sm"
                        className="rounded-full"
                        disabled={addingCode === suggestion.code}
                        onClick={() => void acceptSuggestion(suggestion)}
                      >
                        {addingCode === suggestion.code ? (
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setSuggestions((current) =>
                            current.filter((item) => item.code !== suggestion.code),
                          )
                        }
                      >
        ...
        <div className="flex flex-col gap-4 border-b border-border px-6 py-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 font-display text-xl font-medium">
              {checks.length} custom {checks.length === 1 ? "check" : "checks"}
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Written by your team, run alongside the built-in checks on every scan.
            </p>
          </div>
          {canEdit && (
        ...
        {open && canEdit && (
          <div className="grid gap-4 border-b border-border px-6 py-6 sm:grid-cols-2">
            ...
            <label className="text-sm">
              ...
              <Input
                className="h-10 bg-background"
                placeholder="Internal hostname referenced"
                value={draft.title}
            ...
            <label className="text-sm">
              ...
              <select
                className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                value={draft.layer}
                onChange={(event) => setDraft({ ...draft, layer: event.target.value as Layer })}
              >
                {LAYERS.map((layer) => (
            ...
            <label className="text-sm sm:col-span-2">
              ...
              />
              {patternError && (
                <span className="mt-2 block text-xs text-critical">{patternError}</span>
              )}
            </label>
            <label className="text-sm">
            ...
            <div className="sm:col-span-2 flex flex-wrap items-center gap-4">
              <Button
                className="rounded-full"
                disabled={
                  busy || !!patternError || draft.pattern.length < 2 || draft.title.length < 3
                }
                onClick={() => void submit()}
              >
            ...
            {checks.map((check) => (
              <div key={check.id} className="flex items-start justify-between gap-4 px-6 py-4">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    <span className={severityClass[check.severity as Severity]}>
                      {check.severity}
                    </span>{" "}
                    · {LAYER_LABEL[check.layer as Layer] ?? check.layer} · confidence{" "}
                    {check.confidence}%
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-3">
                  <Switch
                    ...
                    onCheckedChange={async (value) => {
                      await toggle({ data: { id: check.id, enabled: value } });
                      await onChanged();
                    }}
                  />
                  {canEdit && (
      ...
      <section className="rounded-xl border border-border bg-card">
        <div className="flex flex-col gap-4 border-b border-border px-6 py-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 font-display text-xl font-medium">
              {RULES.length} deterministic checks
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Every check, what it looks for, and how precise it is.
            </p>
          </div>
            ...
            return (
              <details key={rule.id} className="group px-6 py-4">
                <summary className="flex cursor-pointer items-center justify-between gap-4 list-none">
                  ...
                  <span className={`shrink-0 text-xs font-medium ${severityClass[rule.severity]}`}>
                    {rule.severity}
                  </span>
                </summary>
                <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                  <p>
                    <span className="text-foreground">Category:</span> {LAYER_LABEL[rule.layer]}
                  </p>
                  <p>
                    <span className="text-foreground">Confidence:</span> {confidence}% —{" "}
                    {confidenceLabel(confidence)} <ConfidenceHint />
                  </p>
                  <p>
                    <span className="text-foreground">Why it matters:</span> {rule.rationale}
                  </p>
                  <p>
                    <span className="text-foreground">How to fix it:</span> {rule.remediation}
                  </p>
                  <div>
                    <p className="text-foreground">Exact text this check looks for:</p>
                    {rule.pattern.source.length > 400 ? (
                      <p className="mt-1.5 text-xs">
                        Structural check — instead of matching text, it inspects the skill's
                        declared details (such as a missing author or licence).
                      </p>
                    ) : (
                      <>
                        <p className="mt-1.5 break-all rounded-md border border-border bg-background p-2.5 font-mono text-xs text-foreground">
                          /{rule.pattern.source}/i
                        </p>
                        <p className="mt-1.5 text-xs">
                          This is the literal search pattern, run line by line and ignoring
                          upper/lower case. <span className="font-mono">|</span> means “or”,{" "}
                          <span className="font-mono">\s+</span> means “one or more spaces”, and{" "}
                          <span className="font-mono">\b</span> means “whole word only”.
                        </p>
                      </>
                    )}
                  </div>
