  ...
  .handler(async ({ context }) => {
    ...
    const [settings, scans] = await Promise.all([
      ...
      context.supabase
        .from("skill_scans")
        .select(
          "id, artifact_name, declared_name, sha256, score, verdict, findings_count, files_count, severity_counts, scanned_at, containment, ai_recommendation, ai_recommendation_reason, ai_recommendation_confidence, recommendation_status, recommendation_decided_at",
        )
        .eq("organization_id", organization.id)
...
export const createWorkspace = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const result = await context.supabase.rpc("create_organization_with_admin", {
      _name: data.name,
      _slug: slug,
    });
...
export const updateRiskSettings = createServerFn({ method: "POST" })
  ...
  .validator((input) =>
    z
      ...
      .refine((value) => value.acceptableScore < value.maliciousScore, {
        message: "Block threshold must be higher than review threshold.",
      })
      .parse(input),
  )
  .handler(async ({ data, context }) => {
  ...
  .handler(async ({ data, context }) => {
    ...
    const inserted = await context.supabase
      ...
      .insert({
        ...
        recommendation_status: autoBlocked
          ? "none"
          : data.recommendation?.action === "quarantine"
            ? "pending"
            : "none",
      })
      .select("id")
    ...
    if (data.findings.length) {
      const findings = await context.supabase.from("scan_findings").insert(
...
export const getScanFindings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const result = await context.supabase
      .from("scan_findings")
      .select(
        "id, rule_id, title, severity, category, file_path, line_number, evidence, remediation, confidence, status, review_note",
      )
...
export const reviewFinding = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (scan.error || findings.error)
      throw new Error("Decision saved, but the scan could not be re-scored.");
    ...
    const confirmedSevere = rows.some(
      (row) =>
        row.status === "confirmed" && (row.severity === "critical" || row.severity === "high"),
    );
    const allDismissed = rows.length > 0 && active.length === 0;
    const containment =
      autoBlocked || confirmedSevere
        ? "quarantined"
        : allDismissed || verdict === "clean"
          ? "cleared"
          : "none";
  ...
  .handler(async ({ data, context }) => {
    const [findings, scans] = await Promise.all([
      context.supabase
        .from("scan_findings")
        .select(
          "id, scan_id, rule_id, title, severity, category, file_path, line_number, evidence, remediation, confidence, status, reviewed_at",
        )
        .eq("organization_id", data.organizationId)
      ...
      context.supabase
        .from("skill_scans")
        .select(
          "id, artifact_name, declared_name, score, verdict, containment, scanned_at, ai_recommendation, ai_recommendation_reason, ai_recommendation_confidence, recommendation_status",
        )
        .eq("organization_id", data.organizationId)
        .order("scanned_at", { ascending: false })
        .limit(200),
    ...
    if (result.error) {
      throw new Error(
        result.error.code === "23505"
          ? "A check with that code already exists."
...
export const suggestCustomChecks = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { SUGGEST_MODEL, SUGGEST_SYSTEM_PROMPT, normalizeSuggestions, suggestUserPrompt } =
      await import("@/lib/check-suggestions");
    ...
    const artifacts = data.artifacts.map((artifact) => ({
      name: artifact.name,
      content: artifact.content.slice(0, budget),
    }));
    const known = [
