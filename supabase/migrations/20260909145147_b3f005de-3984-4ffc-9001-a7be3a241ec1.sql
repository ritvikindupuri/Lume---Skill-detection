CREATE OR REPLACE FUNCTION private.create_organization_with_admin(_name text, _slug text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION private.create_organization_with_admin(text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION private.create_organization_with_admin(text, text) TO authenticated;
...
AS $$
  SELECT private.create_organization_with_admin(_name, _slug)
$$;
